...
var OutputFromObservableRef = class {
    ...
    subscribe(callbackFn) {
        if (this.destroyed) {
            throw new _RuntimeError(953, ngDevMode && "Unexpected subscription to destroyed `OutputRef`. The owning directive/component is destroyed.");
        }
...
function toSignal(source, options) {
    ...
    if (options?.requireSync && state().kind === 0) {
        throw new _RuntimeError(601, (typeof ngDevMode === "undefined" || ngDevMode) && "`toSignal()` called with `requireSync` but `Observable` did not emit synchronously.");
    }
