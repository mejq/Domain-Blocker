...
const parseConfig = (configFile) => {
    const tsCode = (0, getFs_1.default)().readFile(configFile);
    const { outputText } = ts.transpileModule(tsCode, {
        compilerOptions: { module: ts.ModuleKind.NodeNext },
    });
    const userSheriffConfig = eval(outputText);
    if (userSheriffConfig.tagging && userSheriffConfig.modules) {
        throw new user_error_1.TaggingAndModulesError();
    }
    ...
    if (userSheriffConfig.autoTagging === false && !userSheriffConfig.modules) {
        throw new user_error_1.MissingModulesWithoutAutoTaggingError();
    }
    if (userSheriffConfig.encapsulationPattern !== undefined &&
        userSheriffConfig.encapsulatedFolderNameForBarrelLess !== undefined) {
        throw new user_error_1.CollidingEncapsulationSettings();
    }
